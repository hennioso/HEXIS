"""
Trader: Verbindet Strategie, Risk Manager und Exchange.
Führt Orders aus und verwaltet die aktuelle Position.
"""
...
class Trader:
    def __init__(self, client: BitunixClient, risk_manager: RiskManager, symbol: str):
        ...
        self._current_trade_id: Optional[str] = None  # DB-Trade-ID der offenen Position
        db.init_db()

    def refresh_position(self) -> Optional[dict]:
        """Holt die aktuelle offene Position von der Exchange."""
    ...
    def has_open_position(self) -> bool:
        ...
        if pos is None and prev_trade_id:
            # Position wurde extern geschlossen (TP/SL) → DB synchronisieren
            self._sync_closed_position(prev_trade_id)
        return pos is not None

    def _sync_closed_position(self, trade_id: str):
        """Erkennt extern geschlossene Positionen und aktualisiert Status + PnL in der DB."""
        try:
        ...
        # TP oder SL? Aus DB-Eintrag ableiten
        ...
        logger.info(
            f"Position extern geschlossen erkannt | Status: {status} | "
    ...
    def open_position(self, signal: Signal) -> Optional[dict]:
        """
        Öffnet eine neue Position basierend auf dem Signal.
        Gibt die Order-Antwort zurück oder None bei Fehler.
        """
        if self.has_open_position():
            logger.warning("Position bereits offen – kein neuer Trade.")
            return None

        balance = self.get_available_balance()
        if balance < 10:
            logger.error(f"Zu wenig Kapital: {balance:.2f} USDT")
            return None
        ...
        if trade_params is None:
            logger.warning("Positionsgröße zu klein – Trade übersprungen.")
            return None
        ...
        logger.info(
            ...
            f"Qty: {trade_params.qty} BTC | "
            f"~{trade_params.notional_usdt:.0f} USDT Notional | "
            f"Entry: {trade_params.entry_price:.1f} | "
        ...
        try:
            ...
            logger.info(f"Order platziert: {result}")
    ...
    def close_position(self, reason: str = "manual", exit_price: float = None) -> Optional[dict]:
        """Schließt die aktuelle offene Position mit Market Order."""
        pos = self.refresh_position()
        if pos is None:
            logger.info("Keine offene Position zum Schließen.")
            return None
        ...
        logger.info(f"TRADE CLOSE | Grund: {reason} | Qty: {qty} | Side: {close_side}")
        ...
        try:
            ...
            logger.info(f"Position geschlossen: {result}")

            # In Datenbank schließen
            ...
            logger.error(f"Schließen fehlgeschlagen: {e}")
            return None
