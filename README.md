...
- **Equity-based position sizing** — margin per trade = configurable % of total account equity (available + used margin + unrealized PnL); adjustable per user via the dashboard slider
...
# Required — keeps sessions and encrypted API keys stable across restarts
...
This starts the trading bot and the web dashboard together in a single process.

Open [http://localhost:5000](http://localhost:5000) — you will be redirected to the **register** page on first launch. The first account created automatically becomes admin (no invite code required). All subsequent users need an invite code generated by the admin.

#### Running as a persistent background service (Linux / Ubuntu server)

```bash
bash setup.sh          # installs systemd service, creates .env if missing
sudo systemctl start hexis
sudo systemctl status hexis
```
...
| `POSITION_MARGIN_PCT` | 7.5% | % of total equity used as margin per trade (adjustable per user via dashboard slider) |
...
```
HEXIS/
├── run.py                # Unified entry point — dashboard + bot in one process
├── main.py               # Trading bot — all symbol threads + agent scanner
...
├── mailer.py             # SMTP email sender (invite codes)
...
├── database.py           # SQLite trade history, analytics, user accounts
├── web_dashboard.py      # Flask dashboard API + exchange sync
├── hexis.service         # systemd unit file (Linux server auto-start)
